...
- `index.html` — the app itself, with two tabs:
  ...
  - **Week Projections** — baseline projections for whatever NFL week is currently
    upcoming (auto-detected from today's date against the real schedule — advances
    on its own as the season progresses, no manual updates needed), combining last
    season's stats, each player's real current team, their real opponent that week
    (or a "BYE" flag if their team isn't playing), and opponent matchup strength
- `data/players.json` / `data/search_index.json` — real, computed season stats
- `data/projections.json` — the current week's baseline projections
